...
What this builder does:
  ...
  7. Workspace: SpawnLocation at Y=80, StreamingEnabled, Gravity
...
CLIENT_SCRIPTS = {
    "WeatherClient",
    "NPCDialogueClient",
    "AmbienceClient",       # v7.0 - biome ambient crossfade
    "InventoryUI.client",   # v7.0 - inventory grid UI
    "HUD.client",           # HUD (health, gold, biome)
    "MinimapUI.client",     # minimap
    "QuestTracker.client",  # quest tracker overlay
    "DialogueUI.client",    # NPC dialogue UI
}
...
# ── RemoteEvents needed in ReplicatedStorage > ProceduralWorldsRemotes ─────
REPLICATED_EVENTS = [
    "WeatherRemote",
    "NPCDialogueRemote",
    "NotificationRemote",
    ...
    "LevelUpRemote",
    "QuestCompleteRemote",
    "InventoryUpdateRemote",   # v7.0
    "SoundRemote",             # v7.0 - server -> client sound trigger
]
# ── RemoteFunctions ───────────────────────────────────────────────────────
REPLICATED_FUNCTIONS = [
    "SeedRemote",
    "InventoryRemote",         # v7.0 - getSlots / drop / use / equip
]
...
def canonical(filename):
    ...
    # e.g. AmbienceClient.client -> AmbienceClient
...
# CLIENT_SCRIPTS uses canonical names (matching what's in CLIENT_SCRIPTS set)
client_map  = {}
server_map  = {}
...
for cname, (fpath, fname) in name_map.items():
    ...
    if fname == INIT_FILE:
        continue
    # check against CLIENT_SCRIPTS using both canonical and original forms
...
for mod_name in sorted(server_map):
    ...
    print(f"  [SSS ModuleScript]  {mod_name}")
...
for mod_name in sorted(client_map):
    ...
    print(f"  [StarterPlayerScripts LocalScript]  {mod_name}")
...
print(f"  [SS] {len(BIOME_ASSETS)} placeholder asset models in ServerStorage/ProceduralAssets")
...
print("  [WS] SpawnLocation at Y=80")
...
for face in ["Bk","Dn","Ft","Lf","Rt","Up"]:
    _str(sky_p, f"Skybox{face}", f"rbxasset://textures/sky/sky512_{face.lower()}.tex")
...
✅  Build complete!
   ...
   📦  {server_count} server ModuleScripts
   💻  {client_count} client LocalScripts
   🔊  {len(REPLICATED_EVENTS)} RemoteEvents + {len(REPLICATED_FUNCTIONS)} RemoteFunctions
   🌳  {len(BIOME_ASSETS)} placeholder asset models
   ...
   ServerScriptService
     └─ ProceduralWorldsServer  [Script]
          ├─ AnimationManager     [ModuleScript]  (v7.0)
          ├─ SoundManager         [ModuleScript]  (v7.0)
          ├─ InventoryRemote      [ModuleScript]  (v7.0)
          └─ ... all other ModuleScripts
   StarterPlayer → StarterPlayerScripts
          ├─ AmbienceClient       [LocalScript]   (v7.0)
          ├─ InventoryUI.client   [LocalScript]   (v7.0)
          ├─ HUD.client           [LocalScript]
          ├─ MinimapUI.client     [LocalScript]
          ├─ QuestTracker.client  [LocalScript]
          ├─ DialogueUI.client    [LocalScript]
          ├─ WeatherClient        [LocalScript]
          └─ NPCDialogueClient    [LocalScript]
   ReplicatedStorage → ProceduralWorldsRemotes
          ├─ InventoryRemote      [RemoteFunction] (v7.0)
          ├─ SoundRemote          [RemoteEvent]    (v7.0)
          └─ ... all other remotes
   ServerStorage → ProceduralAssets
          └─ (placeholder models for all biome assets + mobs + bosses)
""")
